...
import { buildArchiveFacets } from './components/archiveData'
...
const LayoutArchive = (props) => {
  ...
  const archiveGroups = archivePosts || {}
  const allArchivePosts = Object.values(archiveGroups).flat()
  const archiveFacets = props.archiveFacets || buildArchiveFacets(allArchivePosts)

  return (
    <>
      <div className="mb-10 pb-20 min-h-screen w-full">
        <div className="mb-12 space-y-8">
          <div className="endspace-frame p-6 md:p-8">
            <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <div>
                <div className="tech-text mb-2 text-xs text-[var(--endspace-text-muted)]">
                  INDEX_CONTROL // ARCHIVE_AGGREGATION
                </div>
                <h1 className="text-4xl md:text-6xl font-black text-black tech-text">
                  ARCHIVE
                </h1>
              </div>
              <div className="grid grid-cols-3 gap-4 text-right tech-text">
                <div>
                  <div className="text-2xl font-black text-black">{allArchivePosts.length}</div>
                  <div className="text-[10px] text-[var(--endspace-text-muted)]">POSTS</div>
                </div>
                <div>
                  <div className="text-2xl font-black text-black">{archiveFacets.categories.length}</div>
                  <div className="text-[10px] text-[var(--endspace-text-muted)]">CATEGORIES</div>
                </div>
                <div>
                  <div className="text-2xl font-black text-black">{archiveFacets.tags.length}</div>
                  <div className="text-[10px] text-[var(--endspace-text-muted)]">TAGS</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {archiveFacets.categories.length > 0 && (
            <div className="endspace-frame p-6 md:p-8">
              <div className="mb-4 flex items-center justify-between border-b border-[var(--endspace-border-base)] pb-2">
                <h2 className="text-xl font-black text-black tech-text">CATEGORY</h2>
                <span className="text-xs text-[var(--endspace-text-muted)] tech-text">
                  {archiveFacets.categories.length}_GROUPS
                </span>
              </div>
              <div className="flex flex-wrap gap-3">
                {archiveFacets.categories.map(category => (
                  <SmartLink key={category.name} href={category.href}>
                    <span className="ef-btn group inline-flex">
                      <span className="ef-btn-indicator" />
          ...
          {archiveFacets.tags.length > 0 && (
            <div className="endspace-frame p-6 md:p-8">
              <div className="mb-4 flex items-center justify-between border-b border-[var(--endspace-border-base)] pb-2">
                <h2 className="text-xl font-black text-black tech-text">TAGS</h2>
                <span className="text-xs text-[var(--endspace-text-muted)] tech-text">
                  {archiveFacets.tags.length}_SIGNALS
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {archiveFacets.tags.map(tag => (
                  <SmartLink key={tag.name} href={tag.href}>
                    <span className="inline-flex items-center gap-2 border border-[var(--endspace-border-base)] bg-[var(--endspace-bg-secondary)] px-3 py-1.5 text-xs font-bold uppercase text-[var(--endspace-text-secondary)] transition-colors hover:bg-[#FBFB46] hover:text-black">
                      #{tag.name}
        ...
        {Object.keys(archiveGroups).map((archiveTitle) => (
          <BlogListArchive
            key={archiveTitle}
            archiveTitle={archiveTitle}
            archivePosts={archiveGroups}
          />
        ))}
      </div>
    </>
